...
<template>
  <!-- Added flex container to center the section vertically and horizontally -->
    ...
    <div class="w-full max-w-md px-4 text-center">
      ...
      <div class="flex justify-center space-x-4">
        <a v-for="link in data.socialLinks" :key="link.name" :href="link.url" target="_blank" rel="noopener noreferrer"
          ...
          <span class="sr-only">{{ t(`sections.contact.socialNames.${link.name.toLowerCase()}`, link.name) }}</span>
        </a>
      </div>
    </div>
  </div>
